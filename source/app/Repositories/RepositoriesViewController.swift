...
class RepositoriesViewController: UITableViewController, RepositoriesViewInterface {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let message = self.presenter?.message, let size = self.presenter?.sizeList else {
            return 0
        }
		return self.showMessageTableEmpty(text: message, amount: size, tableView: self.tableView)
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		return presenter.buildCell(to: self.tableView, at: indexPath)
	}
